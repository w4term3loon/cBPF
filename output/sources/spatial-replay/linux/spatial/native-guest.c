...
/* One benign spatial native witness control; stdin authorization follows the native-image receipt. */
...
static int run_control(void)
{
	...
	if (command(BPF_MAP_UPDATE_ELEM, &attr) ||
		...
		goto out;
	printf("CBPF_SPATIAL_NATIVE map map_id=%u map_fd=%d key=1 initial=41 map_readback=%llu map_flags=%u btf_id=%u\n",
	...
	result = 0;
out:
	printf("CBPF_SPATIAL_NATIVE result=%s stage=%s map_id=%u prog_id=%u loaded=%u map_create_count=%u map_update_count=%u map_lookup_count=%u prog_load_count=%u test_run_count=%u info_count=%u retval=%u readback=%llu errno=%d attached=0\n",
