 ...
 * Morello BPF RFC and the corrected CBPF prototype (tree e6c69574c16b).
...
/* The executive gateway and init-only trusted checks use this hybrid ABI. */
...
/* Init-only trusted resolver controls: no program or protected entry runs. */
static bool __init cbpf_guard_check(const char *name, bool use_spill, u64 operation)
{
	struct cbpf_invocation state = { .entered = true };
	volatile uintcap_t spill = 0;
	uintcap_t original = 0, alias = 0;
	bool active = false, failure_return = false, pass = false;
	u32 index;

	local_bh_disable();
	pr_info("CBPF_GUARD case=%s phase=begin native_bpf=0 scope=trusted_resolver\n",
		name);
	if (this_cpu_read(cbpf_active) || refcount_read(&cbpf_object.refs) != 1)
		goto out;
	this_cpu_write(cbpf_active, &state);
	active = true;
	original = cbpf_gate_impl(0, 1, 0);
	if (state.failed || !cheri_tag_get((void *__capability)original) ||
	    state.acquired != 1 || state.released || state.reads || state.cleanup ||
	    refcount_read(&cbpf_object.refs) != 2)
		goto out;
	alias = original;
	if (use_spill) {
		spill = alias;
	...
	if (cbpf_gate_impl(original, 3, 1) != 0 || state.failed)
		goto out;
	if (use_spill)
		alias = spill;
	/* Reject a consumed identity whose public capability remains intact. */
	if (!cheri_tag_get((void *__capability)alias) ||
	    !cheri_is_equal_exact((void *__capability)alias,
				 (void *__capability)state.views[0]) ||
	    cheri_tag_get((void *__capability)state.cells[0].object) ||
	    state.acquired != 1 || state.released != 1 || state.reads ||
	    state.cleanup || refcount_read(&cbpf_object.refs) != 1)
		goto out;
	failure_return = cbpf_gate_impl(alias, operation, 2) == CBPF_FAILURE;
	/* A rejected execution ends here: there are no further gate calls. */
	pass = failure_return && state.failed &&
		cheri_tag_get((void *__capability)alias) &&
		cheri_is_equal_exact((void *__capability)alias,
				     (void *__capability)state.views[0]) &&
		!cheri_tag_get((void *__capability)state.cells[0].object) &&
		state.acquired == 1 && state.released == 1 && !state.reads &&
		!state.cleanup && refcount_read(&cbpf_object.refs) == 1;
out:
	/* Trusted teardown also covers an unexpected acquire/release failure. */
	if (active)
	...
	pr_info("CBPF_GUARD case=%s result=%s acquired=%u released=%u reads=%u cleanup=%u refs=%u public_tag=%u canonical=%u tagA=%u failed=%u failure_return=%u native_bpf=0 scope=trusted_resolver\n",
		name, pass ? "PASS" : "FAIL", state.acquired, state.released,
		state.reads, state.cleanup, refcount_read(&cbpf_object.refs),
		(unsigned)cheri_tag_get((void *__capability)alias),
		(unsigned)cheri_is_equal_exact((void *__capability)alias,
					     (void *__capability)state.views[0]),
		(unsigned)cheri_tag_get((void *__capability)state.cells[0].object),
		state.failed, failure_return);
	for (index = 0; index < ARRAY_SIZE(state.cells); ++index) {
...
/*
 * trusted callback witness: trusted synchronous C callbacks, not BPF subprogram support. One context
 * retains full capability aliases and one invocation owns the entire dispatch.
 * The permanent provider and production gate keep rejection non-destructive.
 */
struct cbpf_callback_context {
	volatile uintcap_t aliases[2];
	u32 calls, completed, value;
	bool independent;
};

static noinline bool __init cbpf_ownership_callback(
		struct cbpf_callback_context *ctx, u32 iteration)
{
	struct cbpf_invocation *s = this_cpu_read(cbpf_active);
	u32 index = ctx->independent ? iteration : 0;
	u64 pc = 4 + 2 * iteration;
	uintcap_t alias = ctx->aliases[index];
	bool canonical = cheri_is_equal_exact((void *__capability)alias,
				(void *__capability)s->views[index]);

	++ctx->calls;
	pr_info("CBPF_CALLBACK phase=enter iteration=%u context=%px state=%px id=%u public_tag=%u canonical=%u private_tag=%u\n",
		iteration, ctx, s, index + 1,
		(unsigned)cheri_tag_get((void *__capability)alias), canonical,
		(unsigned)cheri_tag_get((void *__capability)s->cells[index].object));
	if (!canonical || !cheri_tag_get((void *__capability)alias)) {
		cbpf_fail(s, pc);
		return false;
	}
	if (ctx->independent && iteration == 1) {
		uintcap_t value = cbpf_gate_impl(alias, 2, pc++);

		if (s->failed)
			return false;
		ctx->value = (u32)value;
	}
	cbpf_gate_impl(alias, 3, pc);
	return !s->failed;
}

static noinline bool __init cbpf_dispatch_callbacks(
		struct cbpf_callback_context *ctx,
		bool (*callback)(struct cbpf_callback_context *, u32), u32 limit)
{
	/* Preserve an actual indirect callback call in the linked experiment. */
	bool (*volatile target)(struct cbpf_callback_context *, u32) = callback;
	u32 iteration;

	for (iteration = 0; iteration < limit; ++iteration) {
		if (!target(ctx, iteration))
			return false; /* No next callback or modeled continuation. */
		++ctx->completed;
	}
	return true;
}
...
{
	struct cbpf_invocation state = { .entered = true };
	struct cbpf_callback_context ctx = { .independent = independent };
	bool active = false, dispatched = false, pass = false;
	u32 index, count = independent ? 2 : 1;

	local_bh_disable();
	pr_info("CBPF_CALLBACK case=%s phase=begin context=%px state=%px limit=%u native_bpf=0 scope=trusted_c_dispatch\n",
		name, &ctx, &state, independent ? 2 : 3);
	if (this_cpu_read(cbpf_active) || refcount_read(&cbpf_object.refs) != 1)
		goto out;
	this_cpu_write(cbpf_active, &state);
	active = true;
	for (index = 0; index < count; ++index) {
		ctx.aliases[index] = cbpf_gate_impl(0, 1, index);
		if (state.failed ||
		    !cheri_tag_get((void *__capability)ctx.aliases[index]))
			goto out;
	}
	dispatched = cbpf_dispatch_callbacks(&ctx, cbpf_ownership_callback,
					     independent ? 2 : 3);
	/* Only trusted observations and teardown occur after dispatcher failure. */
	pass = dispatched == independent && state.failed == !independent &&
		ctx.calls == 2 && ctx.completed == (independent ? 2 : 1) &&
		ctx.value == (independent ? 42 : 0) &&
		state.acquired == count && state.released == count &&
		state.reads == (independent ? 1 : 0) && !state.cleanup &&
		refcount_read(&cbpf_object.refs) == 1;
	for (index = 0; index < count; ++index)
		pass = pass && cheri_tag_get((void *__capability)ctx.aliases[index]) &&
			cheri_is_equal_exact((void *__capability)ctx.aliases[index],
					     (void *__capability)state.views[index]) &&
			!cheri_tag_get((void *__capability)state.cells[index].object);
out:
	if (active)
		for (index = 0; index < ARRAY_SIZE(state.cells); ++index)
			if (cheri_tag_get((void *__capability)state.cells[index].object))
				cbpf_consume(&state, index, U64_MAX, true);
	pr_info("CBPF_CALLBACK case=%s result=%s calls=%u completed=%u value=%u acquired=%u released=%u reads=%u cleanup=%u refs=%u failed=%u dispatched=%u native_bpf=0 scope=trusted_c_dispatch\n",
		name, pass ? "PASS" : "FAIL", ctx.calls, ctx.completed, ctx.value,
		state.acquired, state.released, state.reads, state.cleanup,
		refcount_read(&cbpf_object.refs), state.failed, dispatched);
	for (index = 0; index < ARRAY_SIZE(state.cells); ++index) {
		state.cells[index].object = 0;
		state.views[index] = 0;
		ctx.aliases[index] = 0;
...
static int __init cbpf_cap_init(void)
{
	...
	if (!cbpf_guard_check("stale_copy", false, 2) ||
	    !cbpf_guard_check("stale_spill", true, 2) ||
	    !cbpf_guard_check("duplicate_release", false, 3) ||
...
u32 cbpf_test_invoke(struct bpf_prog *prog, void *ctx)
{
	...
	/* Program-accessible registers/sidecar are scrubbed, not all physical copies.
	 * Trusted executive stack saves remain inaccessible to the BPF program. */
