#!/usr/bin/env python3
"""Review only the fixed, benign cbpf_spatial fixture; never load or execute BPF.
...
require the independent source/native review described in docs/results/spatial-native-scope.md.
"""
import argparse
import hashlib
import json
...
def fixed_fixture(map_reference, call_reference):
    return [(0x62,10,0,-4,1), (0xbf,2,10,0,0), (0x07,2,0,0,-4),
            ...
            (0x15,0,0,5,0), (0x71,1,0,6,0), (0x07,1,0,0,1),
            (0x73,0,1,6,0), (0xbf,0,1,0,0), (0x95,0,0,0,0),
            (0xb7,0,0,0,0), (0x95,0,0,0,0)]


def pre(rows, raw_log, args):
    out = args.output
    begin = one(rows, "CBPF_SPATIAL_NATIVE begin ")
    require(begin["program"] == "cbpf_spatial", "unexpected program")
    for k, expected in dict(prog_flags=0, map_flags=0, key=1, key_size=4,
                            value_size=7, max_entries=2, access_offset=6, access_bytes=1).items():
        require(number(begin, k) == expected, "fixture " + k)
    ready = one(rows, "CBPF_SPATIAL_NATIVE_READY ")
    require(number(ready, "executed") == 0, "already executed")
    require(not select(rows, "CBPF_SPATIAL_NATIVE result=") and not select(rows, "CBPF_SPATIAL_NATIVE_VALUE ") and
            not select(rows, "CBPF_SPATIAL_NATIVE_GATE ") and not select(rows, "CBPF_SPATIAL_NATIVE authorization="),
            "result/provider observation exists before review")
    image = one(rows, "CBPF_SPATIAL_NATIVE image ")
    initial = one(rows, "CBPF_SPATIAL_NATIVE map ")
    ...
    require(number(initial, "map_id") == number(ready, "map_id") and
            ...
            number(initial, "map_readback") == 41 and
            initial["value_bytes"] == "11223344556629", "initial map identity/value")
    ...
    original = indexed(select(rows, "CBPF_SPATIAL_NATIVE bpf kind=original "), "pc", 14)
    require([bpf(r) for r in original] == fixed_fixture(number(ready, "held_map_fd"), 1),
            "original BPF is not the fixed benign fixture")
    raw_words = indexed(select(rows, "CBPF_SPATIAL_NATIVE word "), "index", count)
    ...
    require(cert["program"] == "cbpf_spatial" and number(cert, "bpf_len") == 14 and
    ...
    require(decision["program"] == "cbpf_spatial" and number(decision, "accepted") == 1 and
    ...
    require(cert_end["program"] == "cbpf_spatial" and number(cert_end, "covered_words") == count, "certificate end")
    stream = select(rows, "CERT_WORD ") + select(rows, "CERT_IMAGE_WORD ")
    stream = indexed(stream, "native", count)
    require(all(r["program"] == "cbpf_spatial" for r in stream) and
    ...
    require(all(r["program"] == "cbpf_spatial" for r in ranges+sources+states), "certificate program labels")
    ...
    translated = indexed(select(rows, "CBPF_SPATIAL_NATIVE bpf kind=translated "), "pc", 14)
    ...
    checks = {0:[0xd280002a,0xe29fc32a], 1:[0xc2c1d321], 2:[0x02801021],
              ...
              7:[0xe20064e0], 8:[0x91000400], 9:[0xe20060e0], 10:[0xc2c1d007],
              12:[0xd2800007], 13:[]}
    ...
    # The inherited A64_MOV macro emits ADD-immediate zero (bpf_jit.h), not ORR.
    require(prologue[poke-1] == 0x910003c9 and prologue[poke] == 0xd503201f,
    ...
    return dict(result="PASS_PRE_EXECUTION_CHECKS", program="cbpf_spatial", image=image, ready=ready,
                ...
                selected=dict(load_native=number(ranges[7],"start"), store_native=number(ranges[9],"start"),
                              capability="c7", data_register="w0", bytes=1, offset=6,
                              value_size=7, stride=8),
...
def post(rows, raw_log, args):
    ...
    result = one(rows, "CBPF_SPATIAL_NATIVE result=")
    ...
    require(result["value_bytes"] == "1122334455662a", "complete seven-byte readback")
    ...
    authorization = one(rows, "CBPF_SPATIAL_NATIVE authorization=")
    ...
    # Observation schema is deliberately restricted to the two spatial native witness logging sites.
    provider = one(rows, "CBPF_SPATIAL_NATIVE_VALUE ")
    gateway = one(rows, "CBPF_SPATIAL_NATIVE_GATE ")
    order = [next(i for i,r in enumerate(rows) if r.startswith(prefix)) for prefix in
             ("CBPF_SPATIAL_NATIVE authorization=", "CBPF_SPATIAL_NATIVE_GATE ", "ROOT_ATTEST kind=restricted_code ",
              "ROOT_ATTEST kind=executive_return ", "CBPF_SPATIAL_NATIVE_VALUE ", "CBPF_SPATIAL_NATIVE result=")]
    ...
    require(provider["branch"] == "reduced" and number(provider,"map_id") == int(saved["ready"]["map_id"]) and
            number(provider,"index") == 1 and number(provider,"value_size") == 7 and
            number(provider,"stride") == 8,
    ...
    require(number(provider,"selected") == number(provider,"base") == number(provider,"cursor") == selected and
            number(provider,"length") == 7 and number(provider,"tag") == 1 and
    ...
    require(not any(re.search(r"contained BPF|recovered BPF|authority .* rejected|CBPF_SPATIAL_NATIVE.*result=FAIL|Kernel panic|Oops:",r) for r in rows),
            "failure/fault reported")
    return dict(result="PASS_SPATIAL_NATIVE_OBSERVATION", preflight_sha256=sha(args.preflight.read_bytes()),
...
def main():
    ...
    review_root = (PROJECT / "build/spatial-native-review").resolve()
    require(args.output.resolve().is_relative_to(review_root), "output must be inside new build/spatial-native-review")
