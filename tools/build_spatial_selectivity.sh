#!/bin/bash
# Build the default-off synthetic spatial-selectivity profile; execution is separate.
# The production provider is followed by observation and fixed-test overlays.
set -euo pipefail
...
jobs=${CBPF_SPATIAL_SELECTIVITY_BUILD_JOBS:-4}
[[ "$jobs" =~ ^[1-4]$ ]] || { echo 'CBPF spatial selectivity jobs must be 1..4' >&2; exit 2; }
patch=$project/linux/spatial/array-authority.patch
overlay=$project/linux/spatial/native-observation.patch
test_overlay=$project/linux/spatial/selectivity-test.patch
test -r "$patch"
test -r "$overlay"
test -r "$test_overlay"
...
run=$(mktemp -d "$project/build/spatial-selectivity-build.XXXXXX")
mkdir "$run/source" "$run/objects" "$run/receipt" "$run/headers"
trap 'status=$?; if ((status)); then printf "CBPF_SPATIAL_SELECTIVITY_BUILD result=FAIL exit=%s\n" "$status" >"$run/summary.txt"; fi' EXIT
echo "CBPF spatial selectivity build artifacts: $run"
cp "$patch" "$run/receipt/array-authority.patch"
cp "$overlay" "$run/receipt/native-observation.patch"
cp "$test_overlay" "$run/receipt/selectivity-test.patch"
cp "$0" "$run/receipt/build_spatial_selectivity.executed.sh"
...
git -C "$run/source" apply --check --whitespace=error-all "$run/receipt/selectivity-test.patch"
git -C "$run/source" apply --numstat "$run/receipt/selectivity-test.patch" \
    >"$run/receipt/selectivity-test.numstat"
python3 - "$run" <<'PY'
import pathlib, sys
run = pathlib.Path(sys.argv[1])
allowed = {'arch/arm64/mm/extable.c', 'kernel/bpf/Kconfig', 'kernel/bpf/arraymap.c'}
paths = [row.split('\t')[2] for row in
         (run/'receipt/selectivity-test.numstat').read_text().splitlines()]
if set(paths) != allowed or len(paths) != len(allowed):
    raise SystemExit('Selectivity overlay changes unexpected source paths')
PY
git -C "$run/source" apply --whitespace=error-all "$run/receipt/selectivity-test.patch"
python3 - "$run" <<'PY'
...
paths = [row.split('\t')[2] for row in (run/'receipt/patch-numstat.txt').read_text().splitlines()]
paths.extend(row.split('\t')[2] for row in
             (run/'receipt/selectivity-test.numstat').read_text().splitlines())
...
PY
sha256sum "$run/receipt/array-authority.patch" "$run/receipt/native-observation.patch" \
    "$run/receipt/selectivity-test.patch" "$run/receipt/build_spatial_selectivity.executed.sh" \
...
export KBUILD_BUILD_USER=cbpf KBUILD_BUILD_HOST=spatial-selectivity
...
scripts/config --file /build/.config --set-val CAPEBPF_TEST_JIT_CERTIFICATE_MUTATION 0 \
    ...
    --enable CBPF_SPATIAL_SELECTIVITY_TEST \
    --disable CAPEBPF_SPATIAL_OFFLOAD --disable DEBUG_INFO_BTF \
    --disable LOCALVERSION_AUTO --set-str LOCALVERSION "-cbpf-spatial-selectivity"
...
if config.get('CONFIG_CPU_BIG_ENDIAN', 'n') != 'n':
    raise SystemExit('The fixed byte-level fixture requires little endian')
...
make "${args[@]}" -j"$1" arch/arm64/net/bpf_jit_comp.o arch/arm64/mm/extable.o \
    kernel/bpf/arraymap.o kernel/bpf/syscall.o
make "${args[@]}" -j"$1" Image
make "${args[@]}" -j"$1" INSTALL_HDR_PATH=/headers headers_install
cat /build/include/config/kernel.release > /receipt/kernel-release.txt
test "$(cat /receipt/kernel-release.txt)" = '6.7.0-cbpf-spatial-selectivity'
cp /build/include/generated/compile.h /receipt/compile.h
/opt/cheri/output/morello-sdk/bin/llvm-nm -n /build/vmlinux | \
    grep 'cbpf_.*selectivity' > /receipt/selectivity-symbols.txt
/opt/cheri/output/morello-sdk/bin/llvm-objdump -d \
    --disassemble-symbols=cbpf_selectivity_native_access \
    /build/vmlinux > /receipt/selectivity-access-disassembly.txt
...
sha256sum "$run/objects/arch/arm64/net/bpf_jit_comp.o" \
    "$run/objects/arch/arm64/mm/extable.o" "$run/objects/kernel/bpf/arraymap.o" \
    "$run/objects/kernel/bpf/syscall.o" \
...
printf '%s\n' \
    "CBPF_SPATIAL_SELECTIVITY_BUILD result=PASS source_tree=$tree image=1" \
    'CBPF_SPATIAL_SELECTIVITY_BUILD verifier_unchanged_from_substrate=1 offload=0 historical_test_modes=0 selectivity_test=1 observation_sites=2 boot=0' \
    >"$run/summary.txt"
cat "$run/summary.txt"
echo "CBPF retained spatial selectivity build artifacts: $run"
