#!/bin/bash
# Prepare and boot the sole benign spatial native witness control. It waits before TEST_RUN.
# Review READY with check_spatial_native.py before sending CBPF_SPATIAL_NATIVE_EXECUTE on stdin.
set -euo pipefail
project=$(CDPATH= cd -- "$(dirname -- "$0")/.." && pwd)
[[ $# == 1 ]] || { echo 'Usage: run_spatial_native.sh BUILD_DIRECTORY' >&2; exit 2; }
build=$(realpath "$1")
...
assert 'CBPF_SPATIAL_NATIVE_BUILD result=PASS' in (build/'summary.txt').read_text()
assert (receipt/'kernel-release.txt').read_text().strip()=='6.7.0-cbpf-spatial-native'
...
assert (receipt/'build_spatial_native.executed.sh').read_bytes()==(project/'tools/build_spatial_native.sh').read_bytes()
expected={
 'inputs.sha256':{receipt/name for name in ('array-authority.patch','native-observation.patch',
     'build_spatial_native.executed.sh','source-tree.manifest','builder-image.txt')},
...
print('spatial native witness source, kernel, configuration and inherited verifier identities verified')
PY
run=$(mktemp -d "$project/build/spatial-native-run.XXXXXX")
printf '%s\n' "$build" > "$run/build-directory.txt"
cp "$project/linux/spatial/native-guest.c" "$run/guest.c"
cp "$0" "$run/run-script.executed.sh"
cp "$project/tools/check_spatial_native.py" "$run/check_spatial_native.executed.py"
cp "$receipt"/{kernel-release.txt,kernel.config,compile.h} "$run/"
echo "CBPF spatial native witness run artifacts: $run"
...
PY
sha256sum "$run/guest.c" "$run/run-script.executed.sh" "$run/check_spatial_native.executed.py" \
...
args=(-M virt,gic-version=3 -cpu morello -m 2G -smp 1 -bios "$firmware"
      ...
      -append 'console=ttyAMA0 loglevel=7 rdinit=/init panic=-1 capebpf.jit_certificate_transcript=1 sysctl.net.core.bpf_jit_enable=1'
      -nic none -display none -monitor none -serial stdio -no-reboot)
printf '%q ' "$qemu" "${args[@]}" > "$run/qemu-command.sh"
printf '\n' >> "$run/qemu-command.sh"
echo 'Guest will wait at CBPF_SPATIAL_NATIVE_READY; review native admission before sending its execution token.'
set +e
...
echo "CBPF spatial native witness QEMU exit: $status; retain final validation in $run"
exit "$status"
