#!/bin/bash
# Run trusted callback and ownership boundary controls in an offline Morello guest.
set -euo pipefail
project=$(CDPATH= cd -- "$(dirname -- "$0")/.." && pwd)
native=${CBPF_NATIVE_ROOT:-${XDG_CACHE_HOME:-$HOME/.cache}/cbpf/morello}
kernel_build=$project/build/ownership-callback-kernel
headers=$project/build/ownership-callback-headers
receipt=$project/build/ownership-callback-build
...
root = pathlib.Path(sys.argv[1]); receipt = root/'build/ownership-callback-build'
...
for row in (receipt/'outputs.container.sha256').read_text().splitlines():
    ...
    path = root / ('build/ownership-callback-kernel/'+name[7:] if name.startswith('/build/')
                   else 'build/ownership-callback-headers/'+name[9:])
...
require(config == (root/'build/ownership-callback-kernel/.config').read_bytes(), 'configuration receipt')
...
verifier = root/'build/ownership-callback-source/kernel/bpf/verifier.c'
...
run=$(mktemp -d "$project/build/ownership-callback-run.XXXXXX")
...
def require(condition, message):
    if not condition:
        raise SystemExit('CBPF trusted callback witness/ownership boundary controls validation failed: '+message)
...
callback_names = ['retained_release', 'independent_b']
callbacks = sections('CBPF_CALLBACK', callback_names,
    r'CBPF_CALLBACK case=(\w+) phase=begin context=[0-9a-f]+ state=[0-9a-f]+ limit=[23] native_bpf=0 scope=trusted_c_dispatch')
callback_results = []
for name, lines in callbacks.items():
    independent = name == 'independent_b'
    begin = re.fullmatch(r'CBPF_CALLBACK case='+name+r' phase=begin context=([0-9a-f]+) state=([0-9a-f]+) '
                         r'limit=([23]) native_bpf=0 scope=trusted_c_dispatch', lines[0])
    require(int(begin[3]) == (2 if independent else 3), name+' dispatch bound')
    entries = [r for r in lines if r.startswith('CBPF_CALLBACK phase=enter ')]
    require(entries == [f'CBPF_CALLBACK phase=enter iteration={i} context={begin[1]} state={begin[2]} '
            f'id={i+1 if independent else 1} public_tag=1 canonical=1 private_tag={1 if independent else 1-i}'
            for i in range(2)], name+' persistent callback context/identity; no third entry')
    expected = ([(1,'acquire',0,1,1,0,0,0,2,1,0), (2,'acquire',1,2,2,0,0,0,3,1,1),
                 (3,'clear',4,1,2,0,0,0,3,0,1), (4,'release',4,1,2,1,0,0,2,0,1),
                 (5,'read',6,2,2,1,1,0,2,0,1), (6,'clear',7,2,2,1,1,0,2,0,0),
                 (7,'release',7,2,2,2,1,0,1,0,0)] if independent else
                [(1,'acquire',0,1,1,0,0,0,2,1,0), (2,'clear',4,1,1,0,0,0,2,0,0),
                 (3,'release',4,1,1,1,0,0,1,0,0), (4,'reject',6,0,1,1,0,0,1,0,0)])
    require(gates(lines) == expected, name+' exact callback effects')
    bindings(lines, [0,1] if independent else [0])
    count = 2 if independent else 1
    require(lines[-1] == f'CBPF_CALLBACK case={name} result=PASS calls=2 completed={count} '
            f'value={42 if independent else 0} acquired={count} released={count} reads={int(independent)} '
            f'cleanup=0 refs=1 failed={int(not independent)} dispatched={int(independent)} '
            'native_bpf=0 scope=trusted_c_dispatch', name+' callback result')
    callback_results.append({'case':name, 'result':'PASS', 'calls':2, 'completed':count,
        'acquired':count, 'released':count, 'reads':int(independent), 'refs':1,
        'failed':not independent, 'value':42 if independent else 0,
        'native_bpf':False, 'scope':'trusted_c_dispatch'})
# Preserve the ownership boundary controls event checks without confusing trusted callback witness's separately scoped events.
all_rows = rows
rows = []; in_callback = False
for row in all_rows:
    if row.startswith('CBPF_CALLBACK case=') and ' phase=begin ' in row:
        in_callback = True
    if not in_callback:
        rows.append(row)
    if row.startswith('CBPF_CALLBACK case=') and ' result=' in row:
        in_callback = False
require(sum(r.startswith('CBPF_CALLBACK phase=enter ') for r in all_rows) == 4,
        'exactly four trusted callback invocations')
...
(root/'results.json').write_text(json.dumps({'study':'trusted callback witness', 'trusted_callback_checks':callback_results,
    'trusted_resolver_checks':guard_names,
    'cases':results, 'qemu_exit':0, 'invalid_bpf_executions':0,
    'limits':['trusted callback witness executes trusted C callbacks, not BPF callbacks or the original CVE path.',
              'Trusted stale controls do not execute invalid restricted BPF.',
...
summary = '\n'.join(r for r in all_rows if 'CBPF_' in r and not r.startswith(('CBPF_M3 word ','CBPF_M3 bpf ','CBPF_NATIVE word ')))
summary += '\nCBPF ownership_runner=PASS study=trusted_callback_witness qemu_exit=0 executed=4 rejected=3 trusted_checks=3 callback_checks=2\n'
