...
| [Original five-load admission record](../../evidence/current/spatial-selectivity/run/results.json) | Inherited verifier unchanged; two loads admitted, three rejected; zero BPF executions | Admission behavior for the original load-only assessment, not runtime bounds evidence |
| [Later ten-control load/store admission record](../../evidence/current/spatial-selectivity/matrix-v2/results.json) | Same kernel and overlays; five loads plus five stores submitted, four admitted and six rejected; zero BPF executions | Expanded admission coverage, separate from both the original five-load record and the synthetic native observations |
...
| Integrated synthetic native trace | [Three fixed fixtures](ownership-native-trace.md) join alias transport, B's successful read, stale-A rejection and exactly-once cleanup through the production path. | Observed for the trusted native fixtures marked `verified_bpf=0`. Three separate existing BPF admission controls reject at PC 13 for argument shape and never execute; they are not matched counterparts. |
