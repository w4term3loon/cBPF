...
| Whole-value-storage authority does not imply selected-value containment. | **Trust:** the same live two-slot fixture and matched permissions/operands. | **Current observation:** [existing offset-eight result][spatial-counterexample]; exact-seven and stride-eight exclude B's first byte, while both-slot-sixteen permits it. Offset six at widths one and two separately checks the complete access. | Reuses the retained matrix. Sixteen bytes cover both values and padding, not allocation metadata; all roots are capabilities, not a non-capability baseline. |
| Checked construction targets the intended value under the assignment premises. | **Trust:** intact map/provider association, layout, key semantics, allocation lifetime, non-wrapping arithmetic, and correct initial root. **Check:** plain ARRAY, zero flags, expected operations, no record, retained-root shape, and key range. | **Inspection:** [reduced patch][spatial-patch], `cbpf_map_area_alloc` and `cbpf_array_value_cap`. An inconsistent retained allocation root is rejected when it fails the stated shape/association checks. | No new root-corruption experiment. These predicates do not cover every wrong root, authenticate semantic intent or establish allocator-minted provenance. |
| Exact B can permit B's byte when A was intended. | **Trust:** two independent valid production selections and the fixed trusted substitution; recorded intent is distinct from the actual operand. | **Observed, as analytically expected:** [three-load binding control](../results/spatial-selectivity.md#selection-binding-and-the-actual-operand), byte six returns `0x29`, `0x87`, `0x87` at the same native PC; only intended-A/actual-B mismatches. | Correct architectural enforcement of B, with an incorrect intended-value binding. This violates a grant/use premise, not the containment lemma; no reachable BPF exploit or complete mediation follows. |
...
[spatial-counterexample]: ../../theory/spatial.md#per-value-versus-whole-value-storage-authority
