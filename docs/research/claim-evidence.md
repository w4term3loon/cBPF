...
| [Selective spatial enforcement](../results/spatial-selectivity.md) | **Current observation / Inspection:** 30 load/store cases across exact-seven, stride-eight and both-slot roots; 22 permit and eight fault with FSC `0x2a`; five verifier controls execute zero times. | Verifier-admitted invalid eBPF, original-CVE execution, complete mediation, speculative leakage or ownership composition. |
...
| Integrated stale ownership path | Fixed trusted native stale-read and repeated-release cases join identity, rejection, terminal return and exactly-once cleanup. Three corresponding verifier programs are rejected without execution; their diagnostics are not ownership-rule evidence. |
