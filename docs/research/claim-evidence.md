...
Ownership native-image inspection and focused terminal-path checks remain
pending. Publication integrity alone does not discharge those obligations.
...
| Integrated stale ownership path | Fixed trusted native stale-read and repeated-release records join identity, rejection, terminal return and exactly-once cleanup; stronger native inspection is pending. Three separate existing admission controls are rejected without execution; their diagnostics are not ownership-rule evidence. |
