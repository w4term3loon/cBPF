#!/bin/bash
# Build one defensive S5 spatial profile; execution is a separate guarded step.
# The frozen M1 patch is followed only by the two-site observation overlay.
set -euo pipefail
project=$(CDPATH= cd -- "$(dirname -- "$0")/.." && pwd)
tree=e6c69574c16bc2b9bce06329f9ac3f4b3269e79a
source_git=${CBPF_SPATIAL_SOURCE_GIT:-${TEMP_SOURCE}/cheri-ebpf-closure-linux-base/.git}
builder=sha256:b4de3680a00e3ac68ccc56bd87131b97c1fffcd5e016c2d1a54d7e3386d9fc6e
jobs=${CBPF_S5_BUILD_JOBS:-4}
[[ "$jobs" =~ ^[1-4]$ ]] || { echo 'CBPF S5 jobs must be 1..4' >&2; exit 2; }
patch=$project/linux/spatial/array-authority.patch
overlay=$project/linux/spatial/s5-observe.patch
test -r "$patch"
test -r "$overlay"
...
run=$(mktemp -d "$project/build/spatial-s5-build.XXXXXX")
mkdir "$run/source" "$run/objects" "$run/receipt" "$run/headers"
trap 'status=$?; if ((status)); then printf "CBPF_S5_BUILD result=FAIL exit=%s\n" "$status" >"$run/summary.txt"; fi' EXIT
echo "CBPF S5 build artifacts: $run"
cp "$patch" "$run/receipt/array-authority.patch"
cp "$overlay" "$run/receipt/s5-observe.patch"
cp "$0" "$run/receipt/build_spatial_s5.executed.sh"
...
# The S5 delta must contain only the two observation sites.
git -C "$run/source" apply --check --whitespace=error-all "$run/receipt/s5-observe.patch"
git -C "$run/source" apply --whitespace=error-all "$run/receipt/s5-observe.patch"
python3 - "$run" <<'PY'
...
PY
sha256sum "$run/receipt/array-authority.patch" "$run/receipt/s5-observe.patch" "$run/receipt/build_spatial_s5.executed.sh" \
...
export KBUILD_BUILD_USER=cbpf KBUILD_BUILD_HOST=spatial-s5
...
scripts/config --file /build/.config --set-val CAPEBPF_TEST_JIT_CERTIFICATE_MUTATION 0 \
    ...
    --disable LOCALVERSION_AUTO --set-str LOCALVERSION "-cbpf-spatial-s5"
...
test "$(cat /receipt/kernel-release.txt)" = '6.7.0-cbpf-spatial-s5'
...
printf '%s\n' \
    "CBPF_S5_BUILD result=PASS source_tree=$tree image=1" \
    'CBPF_S5_BUILD verifier_unchanged_from_substrate=1 offload=0 test_modes=0 observation_sites=2 boot=0' \
    >"$run/summary.txt"
cat "$run/summary.txt"
echo "CBPF retained S5 build evidence: $run"
