...
/* One benign S5 control; stdin authorization follows the native-image receipt. */
#ifndef __CHERI_PURE_CAPABILITY__
#error "S5 requires the dedicated Morello purecap guest"
#endif
...
#define EXPECTED_RELEASE "6.7.0-cbpf-spatial-s5"
#define PROGRAM_NAME "cbpf_s5"
#define I(C, D, S, O, V) \
	{ .code = (C), .dst_reg = (D), .src_reg = (S), .off = (O), .imm = (V) }

_Static_assert(sizeof(struct bpf_insn) == 8, "Unexpected BPF instruction ABI");
_Static_assert(__BYTE_ORDER__ == __ORDER_LITTLE_ENDIAN__, "S5 requires little endian");
...
static int read_value(int fd, const uint32_t *key, uint64_t *value)
{
	union bpf_attr attr = { 0 };
		      ...
		      unsigned count)
{
	for (unsigned pc = 0; pc < count; ++pc) {
		...
		printf("CBPF_S5 bpf kind=%s pc=%u code=%02x dst=%u src=%u off=%d imm=%d\n",
...
static int run_control(void)
{
	struct bpf_insn program[] = {
		...
		I(BPF_LDX | BPF_MEM | BPF_DW, 1, 0, 0, 0),      /* 7: load */
		I(BPF_ALU64 | BPF_ADD | BPF_K, 1, 0, 0, 1),
		I(BPF_STX | BPF_MEM | BPF_DW, 0, 1, 0, 0),      /* 9: store */
	...
	uint64_t value = 41, readback = 0, native_entry = 0;
	...
	puts("CBPF_S5 begin program=" PROGRAM_NAME
	     " prog_flags=0 map_flags=0 key=1 key_size=4 value_size=8 max_entries=2");
	attr.map_type = BPF_MAP_TYPE_ARRAY;
	attr.key_size = sizeof(key);
	attr.value_size = sizeof(value);
	attr.max_entries = 2;
	memcpy(attr.map_name, "cbpf_s5_map", sizeof("cbpf_s5_map"));
	...
	if (get_info(map_fd, &map, sizeof(map)) || !map.id ||
	    ...
	    map.value_size != 8 || map.max_entries != 2 || map.map_flags ||
	...
	if (command(BPF_MAP_UPDATE_ELEM, &attr) ||
	    read_value(map_fd, &key, &readback) || readback != 41)
		goto out;
	printf("CBPF_S5 map map_id=%u map_fd=%d key=1 initial=41 map_readback=%llu map_flags=%u btf_id=%u\n",
	       map.id, map_fd, (unsigned long long)readback, map.map_flags, map.btf_id);
	...
	printf("CBPF_S5 verifier_begin\n%s\nCBPF_S5 verifier_end\n", verifier_log);
	if (prog_fd < 0)
		goto out;
	...
	for (unsigned index = 0; index < info.jited_prog_len / sizeof(native_words[0]); ++index)
		printf("CBPF_S5 word index=%u value=%08x\n", index, native_words[index]);
	printf("CBPF_S5 image prog_id=%u map_id=%u original_count=%u translated_count=%u native_entry=0x%llx jited_len=%u nr_jited_ksyms=%u nr_jited_func_lens=%u func_len=%u\n",
	...
	stage = "authorization";
	printf("CBPF_S5_READY map_id=%u prog_id=%u held_map_fd=%d held_prog_fd=%d executed=0\n",
	       map.id, info.id, map_fd, prog_fd);
	if (!fgets(authorization, sizeof(authorization), stdin) ||
	    strcmp(authorization, "CBPF_S5_EXECUTE\n"))
		goto out;
	puts("CBPF_S5 authorization=accepted test_run_count=0");
	stage = "test_run";
	memset(&attr, 0, sizeof(attr));
	...
	stage = "map_readback";
	if (read_value(map_fd, &key, &readback) || retval != 42 || readback != 42 ||
	    map_creates != 1 || map_updates != 1 || prog_loads != 1 || test_runs != 1)
		goto out;
	stage = "complete";
	result = 0;
out:
	printf("CBPF_S5 result=%s stage=%s map_id=%u prog_id=%u loaded=%u map_create_count=%u map_update_count=%u map_lookup_count=%u prog_load_count=%u test_run_count=%u info_count=%u retval=%u readback=%llu errno=%d attached=0\n",
	       result ? "FAIL" : "PASS", stage, map.id, info.id, prog_fd >= 0,
	       map_creates, map_updates, map_lookups, prog_loads, test_runs, infos,
	       retval, (unsigned long long)readback, last_errno);
	if (prog_fd >= 0)
		close(prog_fd);
	if (map_fd >= 0)
		close(map_fd);
...
int main(void)
{
	...
	if (getpid() != 1 || uname(&identity) || strcmp(identity.release, EXPECTED_RELEASE)) {
		puts("CBPF_S5 identity=FAIL test_run_count=0 poweroff=0");
		return 1;
	}
	printf("CBPF_S5 kernel=%s pid=1 normal_verifier=1 attached=0\n", identity.release);
	printf("CBPF_S5 kernel_version=%s\n", identity.version);
	result = run_control();
	printf("CBPF_S5 poweroff=begin result=%s\n", result ? "FAIL" : "PASS");
	sync();
	if (reboot(RB_POWER_OFF))
		printf("CBPF_S5 poweroff=FAIL errno=%d\n", errno);
	for (;;)
		pause();
}
