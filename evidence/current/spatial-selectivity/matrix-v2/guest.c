...
static int command(unsigned command, union bpf_attr *attr)
{
	...
	if (command == BPF_MAP_CREATE)
		...
		test_runs++;
		return -1; /* No invocation is authorized, even by a caller mistake. */
	}
	else
		return -1;
...
static int assess_admission(void)
{
	...
	printf("CBPF_SPATIAL_ADMISSION_BEGIN profile=loads-stores-v2 cases=10 key=0 key_size=4 value_size=7 max_entries=2 execution=0\n");
	...
	for (case_index = 0; case_index < sizeof(cases) / sizeof(cases[0]);
		...
		if (test->store) {
			program[7].code = BPF_ST | BPF_MEM |
				(test->width == 1 ? BPF_B : BPF_H);
			program[7].dst_reg = 0;
			program[7].imm = test->width == 1 ? 0xd1 : 0xd2d1;
			program[8] = (struct bpf_insn)I(BPF_ALU64 | BPF_MOV | BPF_K,
						      0, 0, 0, 0);
		}
		...
		failures += !pass;
		printf("CBPF_SPATIAL_ADMISSION_CASE case=%u op=%s offset=%u width=%u expected=%s observed=%s errno=%d executed=0 result=%s\n",
		       case_index, test->store ? "store" : "load", test->offset, test->width,
	...
	close(map_fd);
	if (admitted != 4 || rejected != 6 || map_creates != 1 ||
	    prog_loads != 10 || test_runs || failures)
		failures++;
	printf("CBPF_SPATIAL_ADMISSION_SUMMARY result=%s cases=10 admitted=%u rejected=%u map_create_count=%u prog_load_count=%u executions=%u failures=%u\n",
