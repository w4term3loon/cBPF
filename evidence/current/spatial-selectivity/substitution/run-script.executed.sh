...
# --substitution runs three fixed permitted loads, including one wrong-value operand.
set -euo pipefail
export PYTHONDONTWRITEBYTECODE=1
project=$(CDPATH= cd -- "$(dirname -- "$0")/.." && pwd)
[[ $# == 1 || $# == 2 ]] || {
    echo 'Usage: run_spatial_selectivity.sh BUILD_DIRECTORY [--calibration|--substitution]' >&2
    exit 2
}
mode=matrix
if [[ $# == 2 ]]; then
    case $2 in
        ...
        *) echo 'Only --calibration or --substitution is accepted' >&2; exit 2 ;;
    esac
fi
...
checker=$run/check_spatial_selectivity.executed.py
extra_inputs=()
if [[ $mode == substitution ]]; then
    cp "$project/tools/check_spatial_selectivity.py" "$run/check_spatial_selectivity.py"
    cp "$project/tools/check_spatial_substitution.py" "$run/check_spatial_substitution.executed.py"
    checker=$run/check_spatial_substitution.executed.py
    extra_inputs+=("$run/check_spatial_selectivity.py" "$checker")
fi
...
sha256sum "$run/guest.c" "$run/run-script.executed.sh" \
    ...
    "${extra_inputs[@]}" > "$run/inputs.sha256"
...
elif [[ $mode == substitution ]]; then
    kernel_command+=' cbpf.selectivity_substitution=1'
fi
...
python3 "$checker" --run "$run" --output "$run/results.json"
