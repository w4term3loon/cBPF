...
def metadata(run, mode, profile, origin):
    (run / "validation.json").write_text(json.dumps({
        "schema": ("cbpf-spatial-substitution-inputs-v1" if mode == "substitution"
                   else "cbpf-spatial-validation-inputs-v2"), "mode": mode,
...
def prepare(build, run, mode, calibration):
    if mode in ("matrix", "substitution"):
...
def main():
    ...
    new.add_argument("--mode", choices=("matrix", "calibration", "substitution"), required=True)
