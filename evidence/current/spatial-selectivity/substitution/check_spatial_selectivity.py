...
def relocation(text, command, origin, mode):
    ...
    kaslr = [re.sub(r"^\[\s*\d+\.\d+\]\s*", "", line).strip()
             for line in text.splitlines() if "KASLR" in line]
    if origin == "new-run":
        kernel_message = "KASLR disabled on command line"
        efi_message = "EFI stub: KASLR disabled on kernel command line"
        require("nokaslr" in append and "nokaslr" in boot_options and
                kaslr.count(kernel_message) == 1 and kaslr.count(efi_message) <= 1 and
                all(line in (kernel_message, efi_message) for line in kaslr),
                "Explicit zero relocation; missing or contradictory KASLR basis")
        basis = "explicit nokaslr command and boot observation"
    else:
        require(kaslr == ["KASLR disabled due to lack of seed"] and
...
def validate_calibration_receipt(actual, saved, recorded_inputs):
    # Revalidation may use a newer checker. Preserve the pre-launch receipt's
    # checker identity and bind it to the checker captured by that launch.
    versions = ("checker_sha256", "shared_checks_sha256")
    require({key: value for key, value in actual.items() if key not in versions} ==
            {key: value for key, value in saved.items() if key not in versions},
            "Missing/stale passing calibration receipt")
    require(saved["checker_sha256"] == by_suffix(recorded_inputs, "/check_spatial_selectivity.executed.py") and
            saved["shared_checks_sha256"] == by_suffix(recorded_inputs, "/native_receipt_common.py"),
            "Pre-launch calibration/checker identity mismatch")
...
def validate_run(run, expected_mode=None):
    ...
    return {
        ...
        **({"prelaunch_calibration_sha256": digest(run / "calibration/results.json")} if calibration else {}),
        "scope": {
