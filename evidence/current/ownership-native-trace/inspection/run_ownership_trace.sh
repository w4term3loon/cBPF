...
for option in ('ARM64_BTI_KERNEL', 'ARM64_PTR_AUTH_KERNEL', 'CPU_BIG_ENDIAN'):
    require(settings.get('CONFIG_'+option, 'n') == 'n', 'unreviewed native layout: '+option)
...
cp "$project/tools/ownership_native_checks.py" "$run/ownership_native_checks.py"
cp "$project/tools/prepare_ownership_inspection.py" "$run/prepare_ownership_inspection.executed.py"
cp "$project/tools/package_native_extension.py" "$run/package_native_extension.py"
...
sha256sum "$run/guest.c" "$run/check_ownership_trace.executed.py" "$run/native_receipt_common.py" \
	"$run/ownership_native_checks.py" "$run/prepare_ownership_inspection.executed.py" \
	"$run/package_native_extension.py" \
...
python3 "$run/prepare_ownership_inspection.executed.py" --image "$build/objects/vmlinux" \
	--run "$run" --output "$run/inspection"
python3 "$run/check_ownership_trace.executed.py" --log "$run/boot.log" \
	--inspection-directory "$run/inspection" --run-inputs "$run/inputs.sha256" \
	--kernel-config "$run/kernel.config" \
