...
from ownership_native_checks import fixed_image, linked_receipt, linked_terminal_checks
...
def validate_native(case, lines, output_directory, inspection_directory, linked_ranges):
    ...
    inspection = fixed_image(case, values,
        [(int(match[2]), int(match[3])) for match in maps], bpf, entry, restricted, executive,
        (int(base, 16), int(size), int(exit_base, 16), int(exit_size)),
        inspection_directory / ("native-" + case + "-disassembly.txt"), linked_ranges)
...
def main():
    ...
    parser.add_argument("--inspection-directory", required=True, type=pathlib.Path)
    parser.add_argument("--run-inputs", required=True, type=pathlib.Path)
    ...
    rows = normalize(raw)
    linked_ranges, linked_decode = linked_receipt(args.inspection_directory,
        args.run_inputs, args.kernel_config, raw)
    terminal = linked_terminal_checks(linked_ranges, linked_decode)
    ...
    for name, lines in native.items():
        ...
        native_result, inspection = validate_native(name, lines, args.native_directory,
                                                   args.inspection_directory, linked_ranges)
    ...
    output = {
        ...
        "native_inspection": {
            "images": inspections, "linked_terminal": terminal,
            "checker_sha256": digest(pathlib.Path(__file__)),
            "fixed_checks_sha256": digest(pathlib.Path(__file__).with_name("ownership_native_checks.py")),
            "shared_checks_sha256": digest(pathlib.Path(__file__).with_name("native_receipt_common.py")),
            "input_sha256": {
                "boot.log": digest(args.log), "inputs.sha256": digest(args.run_inputs),
                "kernel.config": digest(args.kernel_config),
                **{path.name: digest(path) for path in sorted(args.inspection_directory.iterdir())
                   if path.is_file() and (path.suffix == ".bin" or path.name in (
                       "linked-ranges.json", "complete-linked-disassembly.txt",
                       *("native-" + name + "-disassembly.txt" for name in NAMES)))},
            },
            "relocation": 0,
            "assurance": "External fixed-image checks plus retained LLVM decoding; separate from encoder self-check and authored native inspection",
        },
