#!/bin/bash
# C1 trusted callbacks plus retained M3 controls in an offline Morello guest.
set -euo pipefail
project=$(CDPATH= cd -- "$(dirname -- "$0")/.." && pwd)
native=${CBPF_NATIVE_ROOT:-${XDG_CACHE_HOME:-$HOME/.cache}/cbpf/morello}
kernel_build=$project/build/ownership-c1-kernel
...
root = pathlib.Path(sys.argv[1]); receipt = root/'build/ownership-c1-build'
...
for row in (receipt/'outputs.container.sha256').read_text().splitlines():
    ...
    path = root / ('build/ownership-c1-kernel/'+name[7:] if name.startswith('/build/')
                   else 'build/ownership-c1-headers/'+name[9:])
...
require(config == (root/'build/ownership-c1-kernel/.config').read_bytes(), 'configuration receipt')
...
verifier = root/'build/ownership-c1-source/kernel/bpf/verifier.c'
...
run=$(mktemp -d "$project/build/ownership-c1-run.XXXXXX")
...
def require(condition, message):
    if not condition:
        raise SystemExit('CBPF C1/M3 validation failed: '+message)
...
# Preserve the M3 event checks without confusing C1's separately scoped events.
all_rows = rows
...
(root/'results.json').write_text(json.dumps({'milestone':'C1', 'trusted_callback_checks':callback_results,
    'trusted_resolver_checks':guard_names,
    'cases':results, 'qemu_exit':0, 'invalid_bpf_executions':0,
    'limits':['C1 executes trusted C callbacks, not BPF callbacks or the original CVE path.',
...
summary += '\nCBPF ownership_runner=PASS milestone=C1 qemu_exit=0 executed=4 rejected=3 trusted_checks=3 callback_checks=2\n'
